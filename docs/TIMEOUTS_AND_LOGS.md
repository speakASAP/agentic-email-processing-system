...
|----------|--------|-------------|
| `lib/ai_client.js` | **15 000 ms (15 s)** | Total request timeout for all AI calls (ingest, classify, extract, decide). `AbortSignal.timeout(TIMEOUT_MS)` and undici `connectTimeout: TIMEOUT_MS`. |
...
So **ingest times out after 15 seconds** if the AI service does not respond in time.
...
So the **15 s limit is on the client (agentic-email)**. If the request never reaches the orchestrator or the response is slow, the client aborts after 15 s.
...
If you **do not** see `Email-triage ingest request received` when the client times out, the request is not reaching the orchestrator (network/DNS/firewall or wrong host/port). If you **do** see it and then a long delay before `Email-triage ingest success`, the hang is inside the orchestrator (we already run ingest in a thread, so next suspect is startup/lifespan or other middleware).
