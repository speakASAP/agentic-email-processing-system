...
async function callAiService(path, body, attempt = 1) {
  ...
  const authHeaders = AI_SERVICE_TOKEN ? { Authorization: `Bearer ${AI_SERVICE_TOKEN}` } : {};
  const opts = {
    method: 'POST',
...
async function checkAiHealth() {
  ...
  const signal = AbortSignal.timeout(timeoutMs);
  const opts = { method: 'GET', signal, headers: {} };
  if (AI_SERVICE_TOKEN) opts.headers['Authorization'] = `Bearer ${AI_SERVICE_TOKEN}`;
