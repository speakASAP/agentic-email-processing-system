...
// Ingest must respond in <1s; use shorter timeout and no retry so we fail fast if unreachable
const INGEST_TIMEOUT_MS = Math.min(5000, parseInt(process.env.AI_INGEST_TIMEOUT_MS || '5000', 10) || 5000);
...
async function callAiService(path, body, attempt = 1) {
  ...
  try {
    ...
    logger.error('AI service request failed', { path, reason, url, error: err.message, attempt, retry: retryable, elapsed_ms: elapsedMs });
    if (retryable) {
