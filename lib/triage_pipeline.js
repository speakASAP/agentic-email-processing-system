...
async function runTriagePipeline(raw, aiClient, logger, callbacks = {}, options = {}) {
  ...
  const useLlmClassifier = options.useLlmClassifier;
  const useLlmDecider = options.useLlmDecider;
  ...
  /** Stage that threw last; used so final "Demo run failed" reports the actual failing stage. */
  ...
  try {
    ...
    try {
      const classifyBody = { payload };
      if (useLlmClassifier !== undefined) classifyBody.use_llm = useLlmClassifier;
    ...
    try {
      const decideBody = { intent, confidence, message_id, tenant_id, entities };
      if (useLlmDecider !== undefined) decideBody.use_llm = useLlmDecider;
