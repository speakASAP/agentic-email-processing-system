  ...
  try {
    ...
    try {
      ...
      if (useLlmClassifier && (classifyResult.model_used === 'rule-based' || !classifyResult.model_used || String(classifyResult.model_used).toLowerCase() === 'rule-based')) {
        if (classifyResult.llm_fallback_reason) {
          logger.warning('LLM requested but classifier used rule-based fallback (graceful degradation)', {
            message_id,
            useLlmClassifier,
            model_used: classifyResult.model_used,
            llm_fallback_reason: classifyResult.llm_fallback_reason.substring(0, 200),
      ...
      if (classifyResult.llm_fallback_reason) classifyDetails.llm_fallback_reason = classifyResult.llm_fallback_reason;
      logger.emitEvent({
        message_id,
        timestamp: ts,
        agent: 'classifier',
    ...
    try {
      ...
      if (useLlmDecider && (decideResult.model_used === 'rule-based' || !decideResult.model_used || String(decideResult.model_used).toLowerCase() === 'rule-based')) {
        if (decideResult.llm_fallback_reason) {
          logger.warning('LLM requested but decider used rule-based fallback (graceful degradation)', {
            message_id,
            useLlmDecider,
            model_used: decideResult.model_used,
            llm_fallback_reason: decideResult.llm_fallback_reason.substring(0, 200),
