/**
 * Email dataset: loads the test dataset (50-email sample) and holds per-email workflow state in memory.
 * Single source of truth is docs/sample_intent_dataset.json (read-only).
 */
...
function load() {
  ...
  if (!Array.isArray(items)) {
    throw new Error('Test dataset must be a JSON array');
  }
  byId = new Map();
...
/**
 * @returns {Array<{ message_id: string, subject: string, bodyPreview: string, status: string, category: string|null, action: string|null, queue: string|null }>}
 */
function getList() {
  const list = [];
  for (const rec of byId.values()) {
    ...
    if (rec.stages.decide && rec.stages.decide.status === STATUS_SUCCESS && rec.stages.decide.action) {
      ...
      queue = rec.stages.decide.queue || null;
    }
    list.push({
...
function setStageResult(message_id, stage, status, data = {}) {
  ...
  Object.assign(base, stage === 'ingest' && { payload: data.payload, model_used: data.model_used },
    stage === 'classify' && { intent: data.intent, confidence: data.confidence, raw_scores: data.raw_scores, model_used: data.model_used, llm_output: data.llm_output },
    stage === 'extract' && { message_id: data.message_id, entities: data.entities, summary: data.summary, model_used: data.model_used },
    stage === 'decide' && { action: data.action, escalation_reason: data.escalation_reason, queue: data.queue, model_used: data.model_used, llm_output: data.llm_output });
