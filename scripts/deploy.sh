...
if [ $DEPLOY_EXIT_CODE -eq 0 ]; then
    ...
    # Check that AEPS container can reach ai-microservice (required for triage to work)
    ...
    if command -v docker >/dev/null 2>&1 && docker ps --format '{{.Names}}' 2>/dev/null | grep -q "^${AEPS_CONTAINER}$"; then
        if ! docker exec "$AEPS_CONTAINER" wget -q -O- --timeout=5 http://ai-microservice:3380/health >/dev/null 2>&1; then
            ...
            echo -e "${YELLOW}╚══════════════════════════════════════════════════════════════════════════════╝${NC}"
            echo -e "${YELLOW}  Triage will show 'AI service unreachable ... timeout' until fixed.${NC}"
    ...
    echo "  7. If the UI shows 'AI service unreachable ... timeout': deploy ai-microservice on this host and ensure only one stack (blue or green) is running; from AEPS container, http://ai-microservice:3380/health must return 200."
    exit 1
fi
