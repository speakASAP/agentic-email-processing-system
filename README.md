...
**If https://aeps.alfares.cz returns 404 after a deploy:** The deploy script no longer removes `aeps.alfares.cz.conf` before the blue/green run (so a failed deploy does not leave the site broken). Re-run `./scripts/deploy.sh` to restore or update the aeps config.
